...
export default function Type() {
  ...
  useEffect(() => {
    if (router.isReady) {
      getQueryAndFindItem();
      Router.events.on("routeChangeComplete", getQueryAndFindItem);
      router.events.on('routeChangeStart', getQueryAndFindItem);
    }
  }, [router.isReady]);

  function getQueryAndFindItem(url) {
    var name;
    if(url){
      name = url.split('/')[2];
    }
    else{
      var newURL = "/Search/" + router.query.name;
      name = newURL.split('/')[2];
    }
  ...
  function FindProducts(type) {
    axios.get(`http://localhost:1000/findProductByType/` + type).then((response) => {
      if (response.data) {
        setDataState(response.data.productFound);
        setGotData(true);
      }
    });
  }
